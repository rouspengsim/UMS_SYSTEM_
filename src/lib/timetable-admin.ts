...
export async function saveClassSchedule({ data }: { data: SaveScheduleInput }) {
  ...
  const incomingClassId =
    ...
    data.classId.trim() &&
    !data.classId.startsWith("student-class-")
      ? data.classId.trim()
      : "";
  ...
  if (!classRow) {
    ...
    classRow = (allClasses ?? []).find((row) => classKey(row.name) === classKey(className)) ?? null;
  }
...
export async function deleteClassSchedule({ data }: { data: DeleteScheduleInput }) {
  const accessToken = requireString(data.accessToken, "Admin session");
