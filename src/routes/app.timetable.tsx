...
export const Route = createFileRoute("/app/timetable")({
  head: () => ({ meta: [{ title: "Timetable — RULE" }] }),
...
function emptyScheduleCells(): Record<(typeof days)[number], ScheduleCell> {
  return days.reduce(
    (acc, day) => {
      acc[day] = { teacher: "", teacherPhone: "", subject: "", room: "" };
      return acc;
    },
                  ...
                  ${
                    slot
                      ? `<div class="slot">
                          ...
                          ${slot.room ? `<div>${escapeHtml(slot.room)}</div>` : ""}
                        </div>`
                      : ""
                  }
                </td>
...
function manualScheduleReportHtml(data: ScheduleBuilderData) {
            ...
            .map((day) => {
              ...
              return `
                <td>
                  ...
                  ${
                    cell.teacher || cell.subject || cell.room
                      ? `<div class="slot">
...
function readDemoClassesMin(): Array<{ id: string; name: string; subject_code: string }> {
  if (typeof window === "undefined") return [];
  try {
    const raw = localStorage.getItem("studentsphere.demo.classes");
    const classes = raw
      ? (JSON.parse(raw) as Array<{ id: string; name: string; subject_code: string }>)
      : [];
    ...
    return [...classItems, ...studentClasses].sort((a, b) => a.name.localeCompare(b.name));
  } catch {
    return [];
  }
}
...
function TimetablePage() {
  ...
  const [manualSchedules, setManualSchedules] = useState<ScheduleBuilderData[]>(() => readManualSchedules());
  const [selectedManualScheduleId, setSelectedManualScheduleId] = useState("");
  const isAdmin = primaryRole === "admin";

  const selectedManualSchedule = manualSchedules.find((schedule) => schedule.id === selectedManualScheduleId) ?? null;
  ...
  const createManualSchedule = () => {
    ...
    toast.success(`Schedule created for ${schedule.className}`);
  };
    ...
    <div>
                  ...
                  {days.map((day) => (
                    <td key={`${row.id}-${day}`} className="px-2 py-3">
                      <div className="grid gap-1.5">
                        <input
                          value={row.cells[day].teacher}
                          onChange={(event) => updateBuilderCell(row.id, day, "teacher", event.target.value)}
                          placeholder="Teacher"
                        ...
                        />
                        <input
                          value={row.cells[day].subject}
                          onChange={(event) => updateBuilderCell(row.id, day, "subject", event.target.value)}
                          placeholder="Subject"
                          className="h-8 rounded-lg border border-border bg-background px-2 outline-none focus:border-primary"
                        />
                        <input
          ...
          <button
            onClick={() =>
              ...
              printDocument(
                ...
                manualScheduleReportHtml(selectedManualSchedule),
              )
            }
      ...
      >
        <div className="mb-5 flex flex-wrap items-end gap-3">
          <Field label="Class">
            <select
              value={selectedManualScheduleId}
              onChange={(event) => setSelectedManualScheduleId(event.target.value)}
              className="h-10 min-w-56 rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary"
            >
              <option value="">Select created schedule</option>
          ...
          <button
            onClick={() => selectedManualSchedule && setScheduleBuilder(selectedManualSchedule)}
            disabled={!selectedManualSchedule}
            className="inline-flex h-10 items-center gap-2 rounded-xl border border-border bg-surface px-4 text-sm font-medium hover:bg-muted disabled:cursor-not-allowed disabled:opacity-50"
          >
            Edit selected
          ...
          <button
            onClick={() => {
              ...
              const next = manualSchedules.filter((schedule) => schedule.id !== selectedManualSchedule.id);
              setManualSchedules(next);
              writeManualSchedules(next);
        ...
        {!selectedManualSchedule ? (
          <p className="rounded-xl border border-dashed border-border py-8 text-center text-sm text-muted-foreground">
            Create a schedule first, then select its class here.
          </p>
        ) : (
          <ManualSchedulePaper schedule={selectedManualSchedule} />
        )}
      </SectionCard>
      {showAdd && (
        <AddTimeShift
...
function ManualSchedulePaper({ schedule }: { schedule: ScheduleBuilderData }) {
  return (
                ...
                {days.map((day) => {
                  ...
                  return (
                    <td key={`${row.id}-${day}`} className="h-24 border border-slate-500 p-0 text-center align-top">
                      ...
                      {(cell.teacher || cell.teacherPhone || cell.subject || cell.room) && (
